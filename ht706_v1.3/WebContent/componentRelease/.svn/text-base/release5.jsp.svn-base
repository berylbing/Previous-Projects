...
<SCRIPT type=text/javascript>
	...
	function deltr() {
		...
		v=document.getElementById("asset.uniqueId").value;
		...
		url=tmp+"/componentRelease/CreatComponentElement.do?method=deleteElementAction&assetId="+v+"&index=";
		for (var i=0; i<trs.length; i++) {
			if (trs[i].checked) {
				url=url+trs[i].value+',';
			}
	...
	showModalDialog("CreatComponentElement.do?method=EditComponentElementAction&elementId="+v+"&assetId="+asset, "","dialogHeight:400px; dialogWidth:400px; help:no");
	location.reload() ;
	//open_window("CreatComponentElement.do?method=EditComponentElementAction&elementId="+v+"&assetId="+asset,400,400);
	}
	function w_open(v){
		v=document.getElementById("asset.uniqueId").value;
	//	alert(v);
		open_window('editX.jsp?assetId='+v, 400,400);
		//location.reload() ;
	//	open_window('componentRelease/editX.jsp?assetId='+v,400,400);
	}
...
<html:hidden property="asset.uniqueId" name="ComponentForm" />
	...
	<DIV class=macrolong_aTab>
		...
		<table width="90%" border="0" align="center" >
		  <tr> 
		    ...
		    <td width="20%"><html:text property="asset.name" name="ComponentForm"/></td>
		  </tr>
		  <tr> 
		    <td align="center">安全分类</td>
		    <td align="left"><html:select name="ComponentForm" property="asset.securityClassification" size="1" >			
		  ...
		  <tr> 
		    ...
		    <td align="center"><html:text property="dateOfInformation" name="ComponentForm" onfocus="calendar(this,null,null,null)" size="11"/></td>
		  </tr>
		  <tr> 
		  ...
		  <tr> 
		    ...
		    <td align="left">
				<nested:write property="asset.approvedState" name="ComponentForm"/>
		    </td>
		...
		<div class=list>
		  <table id=ta1>
		    <tr> 
		      ...
		      <th width="10%">提供日期</th>
		      <th width="10%">操作</th>		      		      			      	      
		    </tr><logic:iterate id="list" name="ComponentForm" property="elements" indexId="index"> 
			    <tr> 
			      <td><html:checkbox name="list" property="elementUniqueId" value='${list.elementUniqueId}' ></html:checkbox></td>
			      ...
			      <td><img onclick="editElement('${list.elementUniqueId}')" src="../images/icon/icon_dianping.gif" width="16" height="16" border="0"></td>
			   	</tr>
		    </logic:iterate>
		  </table>
		</div> 
		...
		<div class=list>
		  ...
		  <table width="90%" id=ta2>
		    ...
		    <logic:iterate id="lista" name="ComponentForm" property="related_Assets">
			    <tr> 
			      <td width="5%" height="26"><input type="checkbox" name="relatedAsset1" value="${lista.targetAsset}" onclick="checkItem('boxb')"></td>
			      <td width="20%">${lista.sourceAsset}</td><!-- 构件名称暂时存在这里  -->
			      <td width="20%">${lista.targetAsset}</td>			      
			      <td width="10%">
			      	<html:select  name="lista" property="relationType" size="1">
						<logic:iterate id="list" name="ComponentForm" property="relationTypes">
							<logic:equal value='${lista.relationType}' name="list" property="relationType">
								<option value='${list.relationName}' selected="selected">${list.relationName}</option>	
							</logic:equal>
							<logic:notEqual value='${lista.relationType}' name="list" property="relationType">
...
<!--成本效益分析-->	
		...
		<div class=list>		        
			<table border="0">
				...
				<tr>
				    <td align='right'>SR(Saving due to avoided cost):</td><td align='left'><html:text property="costeffect.sr" name="ComponentForm"/></td>
				    <td align='left'>
...
<SCRIPT type=text/javascript>
		...
		i=path1.indexOf("tab=");
		//alert(i);
		if(i==-1) initTabs(Array('基本信息','组成元素','分类模式','构件关系','成本效益','其它'),0,'100%',300);
		else{
			initTabs(Array('基本信息','组成元素','分类模式','构件关系','成本效益','其它'),1,'100%',300);
		}
</SCRIPT>

  <br>
  <div id=bc align=center>
  	<input onclick="submitToServer('?method=saveComponentAction&update=true')" type=button value=' 保 存 ' class='input_button'>
	...
	function relation_deltr(n){
		...
		for(i=n;i<checked.length;i++){
			if(checked[i].checked){
				if(checked[i].value==null){
					tb=document.getElementById('ta2').deleteRow(i+1);
					relation_deltr(i);
					break;
				}else{
					Rassets+=(","+checked[i].value);
				}
			}
		}
		path=location.pathname;
		...
		tmp=path.substring(0,index);
		url=tmp+"/componentRelease/CreatComponent.do?method=deleteComponentRelationAction&assetId="
			+document.getElementById("asset.uniqueId").value
			+"&RassetId="+Rassets.substring(1);	
		//alert(url);
		xmlhttp=new ActiveXObject("Microsoft.XMLHTTP");
