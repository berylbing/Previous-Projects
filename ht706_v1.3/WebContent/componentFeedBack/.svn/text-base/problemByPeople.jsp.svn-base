...
<script>	
	...
	function toPage(obj) {
	     location.href = 'problemReport.do?method=listAllProblemReport&page=' + obj.value;
	}
</script>
  ...
  <table border="0" id="ta">
   ...
   <logic:iterate id="problemReport" name="problemReportList" indexId="index">
    <tr>         
      ...
      <td align="center" width='80'>${problemReport.reportDate}</td>
      <td align="center" width='90'>${problemReport.reporter}</td>
...
<div >	
		<font size=2px color="black">(${pagelines*(pageNumbers-1)+1}-${pagelines*pageNumbers}) 共${totalSize}  第${pageNumbers}页/共${totalPage}页</font>
		        ...
		        <%}else{%>
		              <a href="problemReport.do?method=listAllProblemReport&page=1"><font size=2px>首页</font></a>
		        <% }%>
                 <%if(pageNumber<=1){%>
		             <font size=2px color="black">上一页</font>
		        <%}else{%>
		              <a href="problemReport.do?method=listAllProblemReport&page=<%=(pageNumber-1)%>"><font size=2px>上一页</font></a>
		        <% }%>
		        <%if(pageNumber>=totalPage){%>
		             <font size=2px color="black">下一页</font>
		        <%}else{%>
		              <a href="problemReport.do?method=listAllProblemReport&page=<%=(pageNumber+1)%>"><font size=2px>下一页</font></a>
		        <% }%>
		        <%if(pageNumber>=totalPage){%>
		             <font size=2px color="black">末页</font>
		        <%}else{%>
		              <a href="problemReport.do?method=listAllProblemReport&page=<%=totalPage %>"><font size=2px>末页</font></a>
		        <% }%>
